...
Full convergence diagnostics (Rhat, effective sample size, posterior
predictive checks) for every model discussed here are on the
[model diagnostics](https://m-delem.github.io/aphantasiaEmotions/articles/model-diagnostics.html)
page; this page focuses on the comparison itself, not on establishing that
each model converged properly.
...
for: a true baseline, not a straw-man.
...
threshold in advance — sits at VVIQ = `r knot_median` (95% CI [`r
knot_ci_low`, `r knot_ci_high`]), meaningfully below Kvamme et al.'s fixed
