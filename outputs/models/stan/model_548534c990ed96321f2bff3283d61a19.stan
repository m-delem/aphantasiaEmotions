...
transformed parameters {
  ...
  lprior += student_t_lpdf(Intercept | 3, 49, 13.3);
  lprior += student_t_lpdf(sds_1 | 3, 0, 13.3)
...
generated quantities {
  ...
  real prior_Intercept = student_t_rng(3,49,13.3);
  real prior_sds_1 = student_t_rng(3,0,13.3);
  real prior_sigma = student_t_rng(3,0,13.3);
  // use rejection sampling for truncated priors
  while (prior_sds_1 < 0) {
    prior_sds_1 = student_t_rng(3,0,13.3);
  }
  while (prior_sigma < 0) {
