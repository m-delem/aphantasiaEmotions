...
transformed parameters {
  ...
  lprior += student_t_lpdf(Intercept | 3, 13, 5.9);
  lprior += student_t_lpdf(sds_1 | 3, 0, 5.9)
...
generated quantities {
  ...
  real prior_Intercept = student_t_rng(3,13,5.9);
  real prior_sds_1 = student_t_rng(3,0,5.9);
  real prior_sigma = student_t_rng(3,0,5.9);
  // use rejection sampling for truncated priors
  while (prior_sds_1 < 0) {
    prior_sds_1 = student_t_rng(3,0,5.9);
  }
  while (prior_sigma < 0) {
    prior_sigma = student_t_rng(3,0,5.9);
  }
}
