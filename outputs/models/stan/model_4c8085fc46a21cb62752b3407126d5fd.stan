...
transformed parameters {
  ...
  lprior += student_t_lpdf(Intercept | 3, 17, 7.4);
  lprior += student_t_lpdf(sds_1 | 3, 0, 7.4)
    - 1 * student_t_lccdf(0 | 3, 0, 7.4);
...
generated quantities {
  ...
  real prior_Intercept = student_t_rng(3,17,7.4);
  real prior_sds_1 = student_t_rng(3,0,7.4);
  real prior_sigma = student_t_rng(3,0,7.4);
  // use rejection sampling for truncated priors
  while (prior_sds_1 < 0) {
    prior_sds_1 = student_t_rng(3,0,7.4);
  }
  while (prior_sigma < 0) {
    prior_sigma = student_t_rng(3,0,7.4);
  }
}
